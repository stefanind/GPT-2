...
void bias_gelu_forward_cuda(const float* x,
                            const float* bias, 
                            float* out,
                            int batch, 
                            int hidden) {
    int threads = 256; // maybe play with diff thread amounts?
    int blocks = (batch * hidden + threads - 1) / threads; // ensure enough blocks*threads for all data
